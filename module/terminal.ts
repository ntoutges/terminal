...
export class Terminal {
  ...
  private hasMouseMoved: number = 0; // used for detecting if click should trigger focus on input
  private doFocusTimeout: number = null; // used for canceling mousedown if dblclick triggered immediately after
  ...
  constructor(
    ...
    this.els.body.addEventListener("mouseup", () => {
      if (this.hasMouseMoved > 2) return; // moved mouse a small amount
      this.doFocusTimeout = setTimeout(() => { // if dblclick event triggered, allow focus to be canceled
    ...
    this.els.body.addEventListener("click", (e: MouseEvent) => { // triggered after mousedown
      if (e.detail < 2) return; // register double/triple/quadruple/etc. clicks
  ...
  private pushToHistory() {
    if (
      this.els.consoleInput.value.trim().length > 0 && ( // only push if non-empty string
  ...
  private resizeInput() {
    ...
    if (this.getLineCount(bounds.height) > 1) this.els.consoleInput.classList.add("multiline"); // large margin for possible rounding errors
    else this.els.consoleInput.classList.remove("multiline");
  }
  ...
  private getLineCount( height: number ) {
  ...
  private getTextSize(text: string) {
    if (text.length == 0) text = " "; // ensure text is never empty
  ...
  private buildLine(sections: HTMLSpanElement[]) {
    ...
    for (const section of sections) { line.append(section); }
